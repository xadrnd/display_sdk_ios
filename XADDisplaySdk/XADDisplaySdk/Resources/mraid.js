...
console.log = function(msg) {
 ...
 window.webkit.messageHandlers.console.postMessage({message: msg, level: "debug"})
 }
};
 
 mraidLog = function(level, msg) {
 if (typeof enableLog != 'undefined') {
 window.webkit.messageHandlers.console.postMessage({message: msg, level: level})
 }
 };
...
mraid.addEventListener = function(event, listener) {
    ...
    // check to make sure that the listener isn't already registered
      index = listenersForEvent.indexOf(listener)
      if (index > -1) {
          log.i("listener " + listener + " is already registered")
          return
      }
...
mraid.fireStateChangeEvent = function(newState) {
    log.i("mraid.fireStateChangeEvent " + "from " + state + " to " + newState);
    if (state !== newState) {
        state = newState;
